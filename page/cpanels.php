            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">CPANELS</strong>
                    </div>
                    <div class="card-body">
                        <div style="overflow-x:auto;">
                            <table id="bootstrap-data-table-export" class="table-striped table-bordered"
                                ...
                                $query = $db->query("SELECT * FROM `accounts`  WHERE `sold` = '0' AND `category` = '6' ORDER BY RAND()") or die($db->error);

                                while ($row = $query->fetch_assoc()) { // preparing an array
                                    $nestedData = array();
                                    $details = json_decode($row['details'], TRUE);
                                    $cpanel_tld = $details['cpanel_tld'];
                                    $cpanel_ssl = $details['cpanel_ssl'];
                                    $cpanel_detected_hosting = $details['cpanel_detected_hosting'];
                                    echo ' <tr>
                                                <td><center>' . clear($row["item_id"]) . '</center></td>
                                                <td>' . flag($row["country"]) . ' - ' . $row["country_name"] . '</td>
                                                <td>' . $cpanel_tld. '</td>
                                                <td>' . $cpanel_ssl . '</td>
                                                <td>' . $cpanel_detected_hosting . '</td>
                                                <td>'.$row['seller'].'</td>
                                                <td>Check</td>
